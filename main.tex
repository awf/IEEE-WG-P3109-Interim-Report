...
Thus, the unbiased exponent of the largest finite value is $2^w-2$, from which bias is computed as
$$
\emax = (2^w-2) - \bias \implies \bias = (2^w - 2) - (2^{w-1}-1) = 2^{w-1} - 2 + 1 = \emax
$$
For the binary8 formats in this document where $p>1$, only one of the ABOE values is non-finite ($\pm$Infinity), so the unbiased exponent of the largest finite value is $2^w - 1$.
...
% The default bias determines the range of finite values represented by a given format. In IEEE-754 the default bias
% was chosen so that the largest finite exponent Emax was 2��−1. This corresponding to a bias of 2��−1 − 1, accounting
% for the use of the all-bits-one exponent to encode NaNs. With P3109 formats encoding a single NaN, the equivalent
% Emax corresponds to a bias of 2��−1. Concretely, this yields the value tables in Fig 1.
% In contrast to an alternative bias of 2��−1 − 1, the following advantages pertain:
% - All F8E5 finite values are in the Binary16 finite range. (Vs E5 FLT_MAX = 98304.0 > HALF_MAX = 65504.0).
% - The product of two F8E4 values will not exceed HALF_MAX.
% - More values close to zero may be of value in training deep neural networks
% We note that given that most FP8 operations will also allow scaling by powers of two, the precise choice of default
% bias is unlikely to significantly impact training or inference of deep neural network models.
% To vote: FP8 finite values shall be interpreted with a bias of 2��−1
...
A key rationale for inclusion of $-0$ in IEEE-754 was the consistent implementation of branch cuts in the \val{atan2} function~\cite{kahan87,kahanthomas91}.
...
As an aside, sorting using comparison operations, as typically implemented, is undefined in presence of NaNs.
